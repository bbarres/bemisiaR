...
gamme <- read.delim("DataCourbesDR-20180507.txt",dec=".")
...
Smod64<-drm(Eff.morts/Total~Concentration,
            weights=Total,
            data=gamme[which(gamme$Produit=="supreme"& gamme$Population=="P64" ),],
            fct=LN.3u(),
            type="binomial")
...
#pour faire l'analyse de toutes les pops sans aucune distinction
SmodSupreme<-drm(Eff.morts/Total~Concentration,
                 weights=Total,curveid = Population,
                 data=gamme[which(gamme$Produit=="supreme"),],
                 fct=LN.3u(),
                 type="binomial")
plot(SmodSupreme,col=TRUE)

#voilà pour ce qui nous concerne
SmodSupreme<-drm(Eff.morts/Total~Concentration,
           weights=Total,curveid = Population,
           data=gamme[which(gamme$Produit=="supreme" & 
                              gamme$Population %in% c("B","P13","P25")),],
           fct=LN.3u(),
           type="binomial")
plot(SmodSupreme,type="all",pch=c(19,20,23),col=c("black","red","blue"),lwd=3, 
     legendPos=c(0.3,1))

#par contre ça bug sur la représentation "confidence"
plot(SmodSupreme,type="confidence",col=c("black","red","blue"),lwd=3,add=TRUE)
#il ne prend que la "condidence" pour la première régression et plot toujours la 
#même en superposant les couleurs d'où le violet
...
SmodB<-drm(Eff.morts/Total~Concentration,
            weights=Total,
            data=temp[which(temp$Produit=="supreme"& temp$Population=="B" ),],
           fct=LN.3u(),
            type="binomial")

Smod13<-drm(Eff.morts/Total~Concentration,
            weights=Total,
            data=temp[which(temp$Produit=="supreme"& temp$Population=="P13" ),],
            fct=LN.3u(),
            type="binomial")

Smod25<-drm(Eff.morts/Total~Concentration,
            weights=Total,
            data=temp[which(temp$Produit=="supreme"& temp$Population=="P25" ),],
            fct=LN.3u(),
            type="binomial")

plot(SmodB,xlim=c(0,5000),ylim=c(0,1),xlab="Concentration en mg/L",
     ylab="Mortalité",main="Suprême 20 SG / Acétamipride",
     type="obs",broken=FALSE, pch=19,log="x") # Valeurs observées
plot(SmodB,add=TRUE,lwd=5,type="none") # Courbe de régression
plot(SmodB,type="confidence",add=TRUE) # Intervalle de confiance

plot(Smod13,xlim=c(0,5000),ylim=c(0,1),type="obs",
     broken=FALSE,col="Blue",pch=19,add=TRUE) # Valeurs observées
...
plot(Smod25,xlim=c(0,5000),ylim=c(0,1),type="obs",
     broken=FALSE,col="Red",pch=19,add=TRUE) # Valeurs observées
